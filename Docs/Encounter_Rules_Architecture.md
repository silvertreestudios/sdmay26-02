...
Slowed still calculates from Unity-backed controller data, prepared-character and component data are
still read during enrollment, and some scene-compatible manager entry points remain. Treat those
paths as migration seams, not alternative authorities.
...
| Slowed turn-start semantics | Standard rules binding with transitional Unity-backed calculation |
