...
Production mechanisms such as causal-tree settlement, reversible Unity enrollment, restored-effect
adoption, and presentation queues solve current integration requirements. Their existence does not
...
varies per application, such as remaining duration or a rolled value that cannot be derived.
...
- the rules store becomes authoritative once attachment succeeds;
