...
| Encounter lifecycle | Implemented | Rules own explicit initialization, unified combatant addition, initiative, round/cursor, exact turns, action economy, reaction availability, MAP reset, outcome, defeat finalization, and active-effect clocks. Unity presentation waits for causal settlement. |
| Explicit Unity composition and enrollment | Implemented | `UnityEncounterModuleSet`, capability-based `UnityEncounterComposition`, reversible preparation, one normalized `AddCombatantsOp` path for initial and later batches, exact attachments, and one `CompositeLifetime` cleanup boundary are production. |
