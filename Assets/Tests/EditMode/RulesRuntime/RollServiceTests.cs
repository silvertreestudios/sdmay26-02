...
namespace Game.Rules.Runtime.Tests
{
    ...
    public sealed class RollServiceTests
    {
        ...
        public void ScriptedSourceRejectsOverflowWithoutConsumingValues()
        {
            ...
            Assert.Throws<OverflowException>(() => rolls.Roll(new DiceExpression(2, int.MaxValue)));
        ...
        public async Task CallbackRollsAreRecordedAgainstTheirFrameAndShownInDiagnostics()
        {
            ...
            OpResult<int> result = await dispatcher.Dispatch(
                new RecordingRollOp(new DiceExpression(2, 6), DiceExpressions.D20)
            );
            ...
            Assert.That(
                recorded.Select(value => value.Dice),
                Is.EqualTo(new[] { new DiceExpression(2, 6), DiceExpressions.D20 })
            );
            ...
            Assert.That(
                ...
                Does.Contain("roll 1: 2d6 -> [5, 2] total=7")
            );
            Assert.That(
                dispatcher.Diagnostics.Compact,
                Does.Contain("roll 2: 1d20 -> [17] total=17")
            );
        ...
        private sealed class RecordingRollOp : IRuleOp<int>
        {
            ...
            public RecordingRollOp(params DiceExpression[] dice) => Dice = Array.AsReadOnly(dice);
        }
        ...
        private sealed class RecordingRollHandler : IOpHandler<RecordingRollOp, int>
        {
            ...
            public ValueTask<int> Handle(OpFrame<RecordingRollOp> frame, OpHandlerContext context)
            {
