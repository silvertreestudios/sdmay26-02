...
namespace Game.Rules.Runtime.Tests
{
    ...
    public sealed class CheckModifierCollectionTests
    {
        ...
        private static readonly RuleDefinitionId SkillDefinition = new RuleDefinitionId(
            "skill-check-status"
        );
        private static readonly RuleDefinitionId SaveDefinition = new RuleDefinitionId(
            "saving-throw-status"
        );
        ...
        public async Task SkillAndSaveModifiersAreCollectedThroughMiddlewareBeforeRolling()
        {
            ...
            registry
                ...
                .Middleware(RuleLifecyclePhase.Transformation, new SkillStatusMiddleware());
            registry
                .Define(SaveDefinition)
                .Middleware(RuleLifecyclePhase.Transformation, new SaveStatusMiddleware());
            ...
            RuleDispatcher dispatcher = new RuleDispatcherBuilder(
                ...
                .RegisterHandler<CheckWorkflowOp, CheckWorkflowOutcome>(new CheckWorkflowHandler())
                .UseCheckResolution()
                .UseRuleRegistry(registry.Build())
                .Build();

            CheckWorkflowOutcome outcome = RequireResolved(
                await dispatcher.Dispatch(new CheckWorkflowOp())
            ).Value;

            Assert.That(outcome.Skill.Modifiers.Total, Is.EqualTo(8));
            Assert.That(
                outcome.Skill.Modifiers.Applied.Select(value => value.Source),
            ...
            Assert.That(
                outcome.Save.Modifiers.Applied.Select(value => value.Source),
            ...
            : IOpHandler<CheckWorkflowOp, CheckWorkflowOutcome>
        {
            ...
            )
            {
                ...
                CheckOutcome skill = RequireResolved(
                    await context.Dispatch(new SkillCheckOp(Actor, SailingLore, 20, source))
                ).Value;
                CheckOutcome save = RequireResolved(
                    await context.Dispatch(new SavingThrowOp(Actor, SaveKind.Reflex, 20, source))
                ).Value;
            ...
            : IOpMiddleware<CollectSkillCheckModifiersOp, ModifierCollection>
        {
            ...
            )
            {
                ...
                if (
                    ...
                    && frame.Op.Actor == Actor
                    && frame.Op.Skill == SailingLore
                )
                {
