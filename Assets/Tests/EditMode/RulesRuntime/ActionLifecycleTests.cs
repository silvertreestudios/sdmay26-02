...
namespace Game.Rules.Runtime.Tests
{
    ...
    public sealed class ActionLifecycleTests
    {
        ...
        private static readonly SpellSlotPoolId SpellPool = new SpellSlotPoolId(
            "rank-one-prepared"
        );
        ...
        private static readonly RuleDefinitionId BindingDefinition = new RuleDefinitionId(
            "test-binding-definition"
        );
        ...
        public async Task ValidActionCommitsEveryCostBeforeHandlerAndEmitsDeterministicFacts()
        {
            ...
            CountingActionResolvedListener resolvedListener = new CountingActionResolvedListener();
            RuleDispatcher dispatcher = new RuleDispatcherBuilder(
                ...
                .Build();
            TestActionOp operation = new TestActionOp();
            RecordingActionBegunObserver begunObserver = new RecordingActionBegunObserver(handler);
            ...
            Assert.That(
                resolved.Value.DomainSucceeded,
                Is.False,
            ...
            Assert.That(
                dispatcher.Trace.Get<CommitActionCostsOp>(new OpId(11)).ParentId,
                Is.EqualTo(action.Id)
            );
            Assert.That(
                dispatcher.Trace.Get<ActionBegunOp>(new OpId(12)).ParentId,
                Is.EqualTo(action.Id)
            );
            Assert.That(
        ...
        {
            ActionProfile profile = ActionProfile.OneAction(new[] { Trait.FromSlug("attack") });
            InMemoryRulesStore store = CreateFullySeededStore();
            ...
            Assert.That(
                ((InvalidOpResult<TestActionOutcome>)result).Reason,
            ...
            Assert.That(
                dispatcher.Trace.OrderedFrames.Single().OpType,
                Is.EqualTo(typeof(TestActionOp))
            );
        }
        ...
        public async Task FailedLateCostRecheckRollsBackEarlierDraftChangesAtomically()
        {
            ...
            Assert.That(handler.WasCalled, Is.False);
            Assert.That(
                dispatcher.Trace.OrderedFrames.Select(frame => frame.OpType),
                Is.EqualTo(new[] { typeof(TestActionOp), typeof(CommitActionCostsOp) })
            );
        }
        ...
        {
            ActionProfile baseProfile = ActionProfile.Create(ActionCost.Two, Array.Empty<Trait>());
        ...
        public async Task ActionEconomySupportsNoneOneToThreeReactionAndFreeAction()
        {
            ...
            foreach (ActionCostExpectation expectation in cases)
            {
                ...
                Assert.That(
                    result,
                    Is.TypeOf<ResolvedOpResult<TestActionOutcome>>(),
                ...
                Assert.That(
                    result.Facts.OfType<ActionCostSpentFact>().Any(),
        ...
        public void LifecycleOperationsAreNestedOnlyAndEngineConstructed()
        {
            ...
            InvalidOperationException begunError = Assert.ThrowsAsync<InvalidOperationException>(
                async () =>
        ...
        public void BuilderRequiresCompleteActionLifecycleConfiguration()
        {
            ...
            Assert.Throws<InvalidOperationException>(() =>
                duplicateConfiguration.UseActionLifecycle(
                    new FixedActionCatalog(ActionProfile.OneAction(Array.Empty<Trait>()))
                )
            );
        }
        ...
        public void ProfilesAndCostsCopyCallerCollectionsAndRejectInvalidValues()
        {
            ...
            Assert.That(ActionValidationResult.Valid, Is.SameAs(ActionValidationResult.Valid));
            ...
            Assert.Throws<ArgumentException>(() =>
                new ActionProfile(ActionCost.One, new RuleCost[] { null }, Array.Empty<Trait>())
            );
        }
        ...
        {
            RuleRegistryBuilder registry = new RuleRegistryBuilder();
            registry.Define(BindingDefinition).Middleware(RuleLifecyclePhase.Reaction, middleware);
            return registry.Build();
        }
        ...
        {
            public TestActionOutcome(bool domainSucceeded) => DomainSucceeded = domainSucceeded;
        ...
        private sealed class LockObservingProfileResolver : IActionProfileResolver
        {
            ...
            public LockObservingProfileResolver(Func<bool> isDispatcherGateHeld) =>
                this.isDispatcherGateHeld =
                    isDispatcherGateHeld
        ...
        private sealed class RecordingActionHandler : IOpHandler<TestActionOp, TestActionOutcome>
        {
            ...
            )
            {
                ...
                FrequencyUses = context.Snapshot.Frequencies.TryGet(
                    ...
                    ? frequency.Uses
                    : -1;
                return new ValueTask<TestActionOutcome>(new TestActionOutcome(domainSucceeded));
            }
        }
            ...
            : IOpMiddleware<ActionBegunOp, ActionStartOutcome>
        {
            ...
            )
            {
                ...
                CanTriggerReactions = context
                    .Trace.GetActionProfile(frame.Op.ActionOpId)
                    .CanTriggerReactions;
        ...
        private sealed class CountingActionCostListener : IRuleFactListener<ActionCostSpentFact>
        {
            public int Calls { get; private set; }
