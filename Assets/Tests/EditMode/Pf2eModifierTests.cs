...
namespace TestsCombat
{
    ...
    public class Pf2eModifierTests
    {
        ...
        public void SameTypeBonusesAndPenaltiesDoNotStack()
        {
            ...
            Pf2eModifierResolution result = Pf2eModifierResolver.Resolve(
                new[]
                {
                    new Pf2eModifier(
                        1,
        ...
        public void EquippedArmorItemBonusSuppressesLowerItemAcBonus()
        {
            ...
            {
                new ArmorBonus { category = "light", bonus = 4 },
            };
            creature.equippedArmor = new EquipmentArmor
            {
                name = "Leather Armor",
                category = "light",
                acBonus = 2,
        ...
        public void ImportedWeaponActionBonusRemainsUntypedBaseAttackTotal()
        {
            ...
            {
                new WeaponActionBonus { weaponName = "Shortbow", bonus = 7 },
            };

            Pf2eModifierResolution result = creature.ResolveAttackRollForWeapon(
                shortbow,
                new[]
                {
                    new Pf2eModifier(
        ...
        private static void AssertApplied(Pf2eModifierResolution result, params string[] sources)
        {
            foreach (string source in sources)
                Assert.IsTrue(
                    result.AppliedModifiers.Any(modifier => modifier.Source == source),
        ...
        private static void AssertSuppressed(Pf2eModifierResolution result, params string[] sources)
        {
            foreach (string source in sources)
                Assert.IsTrue(
                    result.SuppressedModifiers.Any(modifier => modifier.Source == source),
                    source + " should be suppressed."
                );
        }
    }
}
