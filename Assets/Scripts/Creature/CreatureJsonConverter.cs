...
namespace Game.Creature
{
    ...
    public static class CreatureDtoMapper
    {
        ...
        {
            if (target == null || dto == null || dto.system == null)
                return;
            ...
            target.attackBonus =
                dto.actions != null && dto.actions.Length > 0
                    ? dto.actions[0]?.system?.bonus?.value ?? target.attackBonus
                    : target.attackBonus;

            // TODO: temporary, damage bonus directly from str mod
            target.damageBonus =
                dto.system.abilities != null ? dto.system.abilities.str : target.strMod;

            // Action-specific weapon attack bonuses from imported creature actions
            if (target.weaponActionBonuses == null)
                target.weaponActionBonuses = new List<WeaponActionBonus>();
            ...
            if (dto.actions != null)
            {
                foreach (var action in dto.actions)
                {
                    if (!string.IsNullOrWhiteSpace(action?.name) && action.system?.bonus != null)
                        target.weaponActionBonuses.Add(
                            new WeaponActionBonus
                            {
                                weaponName = action.name,
                                bonus = action.system.bonus.value,
                            }
                        );
                }
            }
            // Weapon attack bonuses by proficiency category
            if (target.weaponBonuses == null)
                target.weaponBonuses = new List<WeaponBonus>();
            target.weaponBonuses.Clear();
            if (dto.weaponBonuses != null)
            {
                target.weaponBonuses.Add(
                    new WeaponBonus { category = "unarmed", bonus = dto.weaponBonuses.unarmed }
                );
                target.weaponBonuses.Add(
                    new WeaponBonus { category = "simple", bonus = dto.weaponBonuses.simple }
                );
                target.weaponBonuses.Add(
                    new WeaponBonus { category = "martial", bonus = dto.weaponBonuses.martial }
                );
                target.weaponBonuses.Add(
                    new WeaponBonus { category = "advanced", bonus = dto.weaponBonuses.advanced }
                );
            }

            // Armor bonuses by proficiency category
            if (target.armorBonuses == null)
                target.armorBonuses = new List<ArmorBonus>();
            target.armorBonuses.Clear();
            if (dto.armorBonuses != null)
            {
                target.armorBonuses.Add(
                    new ArmorBonus { category = "unarmored", bonus = dto.armorBonuses.unarmored }
                );
                target.armorBonuses.Add(
                    new ArmorBonus { category = "light", bonus = dto.armorBonuses.light }
                );
                target.armorBonuses.Add(
                    new ArmorBonus { category = "medium", bonus = dto.armorBonuses.medium }
                );
                target.armorBonuses.Add(
                    new ArmorBonus { category = "heavy", bonus = dto.armorBonuses.heavy }
                );
            }
            ...
            // Saves
            target.fortitudeSave =
                dto.system.saves != null ? dto.system.saves.fortitude : target.fortitudeSave;
            target.reflexSave =
                dto.system.saves != null ? dto.system.saves.reflex : target.reflexSave;
            ...
            if (target.actions == null)
                target.actions = new List<string>();
            ...
            if (target.reactions == null)
                target.reactions = new List<string>();
            ...
            if (target.passives == null)
                target.passives = new List<string>();
            if (target.auras == null)
                target.auras = new List<CreatureAura>();
            // Actions - Passives
            ...
            if (target.equipment == null)
                target.equipment = new List<string>();
            ...
            if (dto.equipment != null)
            {
                foreach (var e in dto.equipment)
                {
                    ...
                    if (
                        e != null
                        && string.Equals(e.type, "ammo", StringComparison.OrdinalIgnoreCase)
                    )
                ...
                foreach (var e in dto.equipment)
                    if (!string.IsNullOrEmpty(e?.name) && !string.IsNullOrEmpty(e?.type))
                    {
                        ...
                        if (e.type.Equals("weapon", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                Debug.LogWarning(
                                    $"CreatureDtoMapper: weapon not found for equipment entry: {e.name}"
                                );
                            }
                            else
                            {
                        ...
                        else if (e.type.Equals("armor", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                Debug.LogWarning(
                                    $"CreatureDtoMapper: armor not found for equipment entry: {e.name}"
                                );
                            }
                            else
                            {
            ...
            if (dto.system?.details != null)
            {
                ...
                if (
                    details.publicNotesParagraphs != null
                    && details.publicNotesParagraphs.Length > 0
                )
                {
        ...
        private static void AddAurasFromPassive(CreatureComponent target, ActionDto passive)
        {
            ...
            foreach (RuleElementDto rule in passive.system.rules)
            {
                if (
                    rule == null
                    || !string.Equals(rule.key, "Aura", StringComparison.OrdinalIgnoreCase)
                    || rule.radius <= 0
                )
                    continue;
                ...
                target.auras.Add(
                    ...
                    {
                        name = passive.name,
                        slug = string.IsNullOrWhiteSpace(rule.slug)
                            ? passive.system.slug
                            : rule.slug,
        ...
        private static int GetBaseSpeed(SpeedEntryDto[] speeds, int defaultValue = 0)
        {
            ...
            foreach (var speed in speeds)
            {
                if (
                    speed != null
                    && string.Equals(speed.type, "value", StringComparison.OrdinalIgnoreCase)
                )
                    return speed.value;
            }
    ...
    public static class CreatureJsonConverter
    {
        ...
        public static GameObject CreateFromFile(string jsonFilePath, GameObject prefab = null)
        {
            ...
            {
                Debug.LogError(
                    $"CreatureJsonConverter: creature not found in Resources at '{resourcePath}'"
                );
                return null;
            }
            ...
            {
                Debug.LogError(
                    $"CreatureJsonConverter: failed to parse creature JSON at Resources/{resourcePath}: {ex.Message}"
                );
                return null;
            }
        ...
        public static EquipmentWeapon GetWeaponByName(string weaponName)
        {
            ...
            {
                Debug.LogWarning(
                    $"CreatureJsonConverter: weapon not found in Resources at '{resourcePath}'"
                );
                return null;
            }
            ...
            {
                Debug.LogError(
                    $"CreatureJsonConverter: failed to parse weapon JSON at Resources/{resourcePath}: {ex.Message}"
                );
                return null;
            }
        ...
        public static EquipmentArmor GetArmorByName(string armorName)
        {
            ...
            {
                Debug.LogWarning(
                    $"CreatureJsonConverter: armor not found in Resources at '{resourcePath}'"
                );
                return null;
            }
            ...
            {
                Debug.LogError(
                    $"CreatureJsonConverter: failed to parse armor JSON at Resources/{resourcePath}: {ex.Message}"
                );
                return null;
            }
        ...
        public static List<EquipmentWeapon> GetAllWeapons()
        {
            ...
            {
                Debug.LogWarning(
                    "CreatureJsonConverter: no equipment assets found in Resources/Datafiles/Equipment"
                );
            ...
            foreach (var asset in assets)
            {
                ...
                {
                    Debug.LogError(
                        $"CreatureJsonConverter: failed to parse weapon JSON in Resources asset '{asset.name}': {ex.Message}"
                    );
                    continue;
                }
                if (dto.type == "weapon")
                {
                    ...
                    weapon.materialGrade = dto.materialGrade;
                    weapon.runes = dto.runes;
                    weapon.price = double.TryParse(dto.price, out double priceValue)
                        ? priceValue
                        : 0.0; // Handle parsing price string to double
                    weapon.range = dto.range;
                    weapon.reload = dto.reload;
                    weapon.ammo = dto.ammo;
                    weapon.bulk = dto.bulk;
        ...
        public static List<EquipmentArmor> GetAllArmors()
        {
            ...
            foreach (var asset in assets)
            {
                ...
                {
                    Debug.LogError(
                        $"CreatureJsonConverter: failed to parse armor JSON in Resources asset '{asset.name}': {ex.Message}"
                    );
                    continue;
                }
                if (dto.type == "armor")
        ...
        private static void PopulateSparseSkillsFromJson(string jsonText, CreatureDto dto)
        {
            ...
            string skillsObjectBody = jsonText.Substring(
                objectStart + 1,
            ...
            foreach (Match match in pairMatches)
            {
                ...
                {
                    dto.system.skills.Add(
                        new SkillValue { skillName = skillName, skillMod = skillMod }
                    );
                }
            }
        }
