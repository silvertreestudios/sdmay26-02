...
namespace Game.DungeonGeneration
{
    ...
    public sealed class DungeonLevelParseResult
    {
        ...
        public DungeonLevelParseResult(
    ...
    public static class DungeonLevelJsonParser
    {
        ...
        public static DungeonLevelParseResult Parse(string json)
        {
            ...
            List<DungeonCell> safe = ReadCells(
                arrival?["safeCells"] as JArray,
                "arrival.safeCells",
                errors
            );
            List<DungeonObjectPlacement> objects = ReadObjects(root["objects"] as JArray, errors);
            List<DungeonEncounterPlan> encounters = ReadEncounters(
                root["encounterPlans"] as JArray,
                errors
            );
        ...
        private static DungeonGenerationMetadata ReadGeneration(
            JObject source,
        ...
        )
        {
            ...
            if (depth < 0)
                errors.Add(D("generation.depth", "Depth must be zero or greater."));
            if (topologyAttempt < 0)
                errors.Add(
                    D("generation.topologyAttempt", "Topology attempt must be zero or greater.")
                );
            DungeonGenerationMetadata metadata = new(algorithm, runSeed, depth, topologyAttempt);
            if (
                DeterministicDungeonGenerator.OwnsContract(metadata)
        ...
        private static List<DungeonRoom> ReadRooms(
            JArray array,
            List<DungeonGenerationDiagnostic> e
        ) =>
            ReadObjects(
                array,
                "rooms",
                ...
                (o, p) =>
                {
                    ...
                    return new DungeonRoom(
                        RequiredInt(o, "id", p, e),
                        RequiredInt(o, "minX", p, e),
                        RequiredInt(o, "minZ", p, e),
                        RequiredInt(o, "maxX", p, e),
                        RequiredInt(o, "maxZ", p, e)
                    );
                }
            );

        private static List<DungeonDoor> ReadDoors(
            JArray array,
            List<DungeonGenerationDiagnostic> e
        ) =>
            ReadObjects(
                ...
                (o, p) =>
                {
                    ...
                    return new DungeonDoor(
                        ...
                        RequiredBool(o, "isOpen", p, e)
                    );
                }
            );

        private static List<DungeonStair> ReadStairs(
            JArray array,
            List<DungeonGenerationDiagnostic> e
        ) => ReadObjects(array, "stairs", e, (source, path) => ReadStair(source, path, e));

        private static List<DungeonObjectPlacement> ReadObjects(
            JArray array,
            List<DungeonGenerationDiagnostic> e
        ) =>
            ReadObjects(
            ...
            ReadObjects(
                ...
                (source, path) =>
                {
                    ...
                    return new DungeonEncounterPlan(
                        ...
                        ReadThreat(
                            RequiredString(source, "threat", path, errors),
                            path + ".threat",
                            errors
                        ),
        ...
        )
        {
            ...
            return new DungeonRuntimeState(
                ...
                ReadStrings(
                    source["resolvedEncounterIds"] as JArray,
                    "runtimeState.resolvedEncounterIds",
                    errors
                ),
                ReadStrings(
                    source["defeatedCreatureIds"] as JArray,
        ...
        private static List<DungeonCreatureRuntimeState> ReadCreatures(
            JArray array,
            List<DungeonGenerationDiagnostic> e
        ) =>
            ReadObjects(
                ...
                (o, p) =>
                {
                    ...
                    return new DungeonCreatureRuntimeState(
                        RequiredString(o, "instanceId", p, e),
                        RequiredString(o, "creatureId", p, e),
                        RequiredString(o, "encounterId", p, e),
                        ReadCell(o["cell"], p + ".cell", e),
                        RequiredInt(o, "hitPoints", p, e),
                        OptionalString(o, "state", p, e)
                    );
                }
            );
        ...
        )
        {
            ...
            errors.Add(
                D(path + "." + name, "The optional value must be a finite number when present.")
            );
            return 0f;
        }
        ...
        {
            bool InBounds(DungeonCell c) =>
                c.X >= 0 && c.Z >= 0 && c.X < rows[0].Length && c.Z < rows.Count;
                ...
                for (int left = 0; left < rooms.Count; left++)
                {
                    ...
                    for (int z = room.MinimumZ; z <= room.MaximumZ; z++)
                    {
                        for (int x = room.MinimumX; x <= room.MaximumX; x++)
                        {
                            ...
                            {
                                errors.Add(
                                    D("rooms", "Every cell inside room bounds must contain '.'.")
                                );
                            }
                        }
                    }
            ...
            {
                if (!doorCells.Add(door.Cell) || !InBounds(door.Cell) || Symbol(door.Cell) != 'D')
                {
            ...
            if (ownsContract && !DungeonTopologyValidator.HasProducibleDoorRecords(doors))
            {
                errors.Add(
                    D(
                        "doors",
            ...
            bool invalidStairs =
                stairs.Select(stair => stair.Id).Distinct(StringComparer.Ordinal).Count()
                    != stairs.Count
                ...
                || stairs.Any(stair =>
                    ...
                    || Math.Abs(stair.Cell.X - stair.ArrivalCell.X)
                        + Math.Abs(stair.Cell.Z - stair.ArrivalCell.Z)
                        != 1
                );
            if (invalidStairs)
            {
            ...
            if (ownsContract && !DungeonTopologyValidator.HasProducibleStairRecords(stairs))
            {
                errors.Add(
                    D(
                        "stairs",
            ...
            if (
                ownsContract
                && !DungeonTopologyValidator.HasProducibleSafeCells(rows, rooms, stairs, safe)
            )
            {
                errors.Add(
                    D(
            ...
            bool invalidObjects =
                objects.Select(item => item.Id).Distinct(StringComparer.Ordinal).Count()
                    != objects.Count
                || objects.Any(item =>
                    !InBounds(item.Cell)
                    || (
            ...
            bool invalidEncounterIds =
                ...
                || encounters.Select(plan => plan.Id).Distinct(StringComparer.Ordinal).Count()
                    != encounters.Count;
            ...
            foreach (DungeonEncounterPlan plan in encounters)
            {
                ...
                bool valid =
                    ...
                    && (
                        !ownsContract
                        || plan.SpawnCells.All(cell => !reservedEncounterCells.Contains(cell))
                    );
                if (!valid)
                {
                    errors.Add(
            ...
            if (runtime == null)
            {
                ...
                {
                    errors.Add(
                        D("doors", "Door open flags must be false when runtime state is absent.")
                    );
                }
                ...
                if (encounters.Any(plan => plan.IsResolved))
                {
                    errors.Add(
                        D(
                            "encounterPlans",
                            "Encounter resolved flags must be false when runtime state is absent."
                        )
                    );
                }
            }
            ...
            {
                HashSet<string> doorIds = new(
                    doors.Select(door => door.Id),
                    StringComparer.Ordinal
                );
                HashSet<string> encounterIds = new(
                    encounters.Select(plan => plan.Id),
                    StringComparer.Ordinal
                );
                ...
                if (
                    openDoorIds.Count != runtime.OpenDoorIds.Count
                ...
                )
                {
                    errors.Add(
                        D(
                            ...
                            "Open door IDs must be unique and reference generated doors."
                        )
                    );
                }
                ...
                if (
                    resolvedEncounterIds.Count != runtime.ResolvedEncounterIds.Count
                ...
                )
                {
                    errors.Add(
                        D(
                            "runtimeState.resolvedEncounterIds",
                            "Resolved encounter IDs must be unique and reference encounter plans."
                        )
                    );
                }
                ...
                {
                    errors.Add(
                        D("runtimeState.creatures", "Live creature instance IDs must be unique.")
                    );
                }
                if (
                    runtime.Creatures.Select(creature => creature.Cell).Distinct().Count()
                    != runtime.Creatures.Count
                )
                {
                    errors.Add(
                        D("runtimeState.creatures", "Live creature occupied cells must be unique.")
                    );
                }
                ...
                Dictionary<string, DungeonEncounterPlan> encounterById = new(
                    StringComparer.Ordinal
                );
                foreach (DungeonEncounterPlan plan in encounters)
                {
                    if (!string.IsNullOrWhiteSpace(plan.Id) && !encounterById.ContainsKey(plan.Id))
                ...
                bool exceedsPlannedMultiplicity = runtime
                    .Creatures.GroupBy(creature => (creature.EncounterId, creature.CreatureId))
                    .Any(group =>
                    {
                        if (
                        ...
                        int plannedCount = plan.CreatureIds.Count(creatureId =>
                            string.Equals(
                                creatureId,
        ...
        private static DungeonGenerationDiagnostic D(string field, string message) =>
            new(DungeonGenerationDiagnosticCode.InvalidDocument, field, message);

        private static DungeonLevelParseResult Invalid(string field, string message) =>
            new(null, new[] { D(field, message) });
    }
}
