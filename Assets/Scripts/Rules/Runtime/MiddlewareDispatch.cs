...
namespace Game.Rules.Runtime
{
    internal sealed class BoundMiddlewareRegistration
    {
        ...
        public static int Compare(
            BoundMiddlewareRegistration left,
        ...
        )
        {
            ...
            int id = string.Compare(
                left.Binding.Id.Value,
                right.Binding.Id.Value,
                StringComparison.Ordinal
            );
            if (id != 0)
                return id;
            return left.Registration.RegistrationOrder.CompareTo(
                right.Registration.RegistrationOrder
            );
        }
    }
        ...
        )
        {
            if (!(invocation is FrameInvocation<TOp> typed))
                throw new InvalidOperationException(
                    "Middleware received an incompatible operation frame."
                );

            CallbackWorkCoordinator work = new CallbackWorkCoordinator();
            MiddlewareContinuation<TResult> continuation = new MiddlewareContinuation<TResult>(
                next,
                work
            );
            ...
            try
            {
                result = await middleware.Invoke(typed.Frame, context, continuation.Invoke);
            }
            catch (Exception callbackException)
            {
                await CallbackFailure.AwaitCleanupPreservingPrimary(
                    callbackException,
                    work.CompleteInvocation("A middleware callback completed more than once.")
                );
                throw;
            }
            ...
            if (result == null)
                throw new InvalidOperationException(
                    $"Middleware for {typeof(TOp).Name} returned a null result."
                );
            return result;
        }
    }
    ...
    internal sealed class MiddlewareContinuation<TResult>
    {
        ...
        public MiddlewareContinuation(Func<ValueTask<object>> next, CallbackWorkCoordinator work)
        {
            this.next = next ?? throw new ArgumentNullException(nameof(next));
            this.work = work ?? throw new ArgumentNullException(nameof(work));
        }

        public ValueTask<OpResult<TResult>> Invoke() => work.StartContinuation(InvokeNext);
